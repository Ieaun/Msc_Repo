namespace SVM.VirtualMachine
{
    ...
    internal static class JITCompiler
    {
        ...
        internal static IInstruction CompileInstruction(string opcode)
        {
            ...
            // first find all Dlls in directory
            //if (dll_Assemblies.Count == 0)
            //{
            //    try
            //    {
            ...
            //    }
            //}
            if (dll_Assemblies.Count == 0) // if still 0 then add null so it doesnt search again
            {
            ...
            // load all assemblies
            Type[] As_Types = Assembly.GetExecutingAssembly().GetTypes();
            foreach (Type T in As_Types)
            {
                    ...
                    foreach (string dll in Directory.GetFiles(Environment.CurrentDirectory, "*.dll"))
                    {
                        ...
                        foreach (string section in Temp)
                        {
                            if (section.Contains("publickey"))
                            {
                                ...
                                {
                                    dll_Assemblies.Add(A);
                                    Console.WriteLine("Has Strong name---------->  " + section + " " + Temp[0]); 
                                }
                            }
                        }
                    }
