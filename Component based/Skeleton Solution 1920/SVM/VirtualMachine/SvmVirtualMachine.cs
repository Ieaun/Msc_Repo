...
namespace SVM
{
    ...
    public sealed class SvmVirtualMachine : IVirtualMachine
    {
        ...
        private void Run()
        {
            ...
            SvmVirtualMachine virtualMachine = this;//new SvmVirtualMachine();
            ...
            while (programCounter < program.Count)
            {
                ...
                {
                    //checked if line with breakpoint
                    if (programCounter == LinesWithBreakPoints[breakpoint_index]) //breakpoint_index = counter for the list
                    {
                        ...
                        if (debugger != null)
                        {
                            ...
                            //instruct.Run(); // run previous instruction ?
                            Console.WriteLine("Current Instruct: " + Currrent_instruct); // current

                            ////first tiem debug = open new debug window, not = update opened window
                            int First_time_debug;
                            ...
                            virtualMachine.Stack.Push(First_time_debug); // so ui can see what to do
                            debugger.VirtualMachine = virtualMachine;
                           
                            debugger.Break(D_data);

                            virtualMachine.Stack.Pop(); // pop off First_time_debug
